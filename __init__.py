...
def us(adata,gene,groups=None, show=False, exclude =None,**kwargs):
    if ',' in gene:
        gene = gene.split(',')
    if groups:
...
def save_html(name,log=False): # export notebook into html file
    ...
    if log:
        print(os.system('mv %s.html %s_%s_%s.html'%(name_key,name_key,timestamp,log)))
    else:
        print(os.system('mv %s.html %s_%s.html'%(name_key,name_key,timestamp)))
